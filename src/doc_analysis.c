...
State    state_transition(DocAnalysis *doc_features, struct token_buf *buf, Alphabet symbol, State cur_state, char *c);
int      push_char(struct token_buf *buf, char c);
int      make_token(DocAnalysis *doc_features, struct token_buf *buf);
char     *digit_to_char(char *c);
Alphabet get_symbol(DocAnalysis *doc_features, State current_state, char *c);

/*
 *
 * Returns a doc_analysis structure or NULL if there are memory errors.
 */
DocAnalysis *
analyze_doc(char *label, char *doc)
{
    ...
    /* Initialize the doc_analysis structure. */
    doc_features = (DocAnalysis *)malloc(sizeof(DocAnalysis));
...
void
doc_analysis_free(DocAnalysis *doc)
{
...
State
state_transition(DocAnalysis *doc_features, struct token_buf *buf,
...
int
make_token(DocAnalysis *doc_features, struct token_buf *buf)
{
...
Alphabet
get_symbol(DocAnalysis *doc_features, State current_state, char *c)
{
    Alphabet cur_symbol;
